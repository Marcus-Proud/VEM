...
function buildFieldData(job, staffByEmail) {
  ...
  const staffId1 = contact1?.email ? staffByEmail.get(contact1.email.toLowerCase()) ?? null : null;
  const staffId2 = contact2?.email ? staffByEmail.get(contact2.email.toLowerCase()) ?? null : null;
  ...
  return {
    ...
    datum: job.publishing_date ? new Date(job.publishing_date).toISOString() : null,
...
async function createItem(fieldData) {
  return webflowRequest("POST", `/collections/${WEBFLOW_COLLECTION_ID}/items`, {
    fieldData,
    isDraft: false,
  });
}
...
async function sync() {
  ...
  console.log(`   ${staffItems.length} medarbetare hittade (${staffByEmail.size} med e-post)\n`);
  ...
  if (toPublish.length > 0) {
    ...
    // Publicera i batchar om 100 (Webflow-gräns)
